...
dataESG= initial_dataset[['Security ISIN','Implied Temperature Rise [Â°C]','ESG_NEW']]
dataESG['Implied Temperature Rise [Â°C]']=dataESG['Implied Temperature Rise [Â°C]'].fillna(2)


# DÃ©finir les paramÃ¨tres nÃ©cessaires
num_assets = len(dataESG)  # Nombre d'actifs dans le portefeuille
...
def fitness(solution):
    ...
    weighted_esg_score = np.dot(solution, dataESG['ESG_NEW'])
    
    # Calcul de la température implicite moyenne pondérée
    weighted_temp_rise = np.dot(solution, dataESG['Implied Temperature Rise [Â°C]'])
    ...
    # Fonction de fitness : maximisation du score ESG avec pénalités
    fitness_value = weighted_esg_score - (penalite1 + penalite2 + penalite3)
...
stratégie2 = pd.DataFrame({
    'Security ISIN': dataESG['Security ISIN'],
    'Poids (%)': best_solution
})


# Calculer les métriques pour le meilleur portefeuille
optimal_esg_score = np.dot(best_solution, dataESG['ESG_NEW'])
optimal_temp_rise = np.dot(best_solution, dataESG['Implied Temperature Rise [Â°C]'])
...
stratégie2 = pd.merge(stratégie2, var_viz, on = 'Security ISIN', how = 'left')
...
for var in var_viz:
    if var not in var_non_appplicable:
        stratégie2[var] = stratégie2[var] * stratégie2['Poids (%)']
...
stratégie2 = pd.merge(stratégie2, var_desc, on = 'Security ISIN', how = 'left')

stratégie2 = stratégie2.rename(columns={'Poids (%)': 'Poids'})
stratégie2 = stratégie2.rename(columns={'GICS Sector': 'Secteur'})
stratégie2 = stratégie2.rename(columns={'Security ISIN': 'ISIN'})
stratégie2 = stratégie2.rename(columns={'Aggregated Security Climate VaR [%]': 'Climate_VaR'})
# stratégie2 = stratégie2.rename(columns={'Fluvial Flooding Security Climate VaR [%]': 'Fluvial_flooding_VaR'})
# stratégie2 = stratégie2.rename(columns={'Fluvial Flooding Cost/Profit [USD]': 'Fluvial_flooding_VaR'})

nom_table = 'stratégie_2'
stratégie2.to_sql(nom_table, con=connection_string, if_exists='replace', index=False)
